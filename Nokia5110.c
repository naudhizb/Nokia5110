/*
 * Nokia5110.c
 *
 *  Created on: Jan 2, 2023
 *      Author: naudhizb
 */

#include "Nokia5110p.h"
#include "Nokia5110.h"
#include <string.h>

const unsigned char charset[][5] = {
  { 0x00, 0x00, 0x00, 0x00, 0x00 },  // 20 space
  { 0x00, 0x00, 0x5f, 0x00, 0x00 },  // 21 !
  { 0x00, 0x07, 0x00, 0x07, 0x00 },  // 22 "
  { 0x14, 0x7f, 0x14, 0x7f, 0x14 },  // 23 #
  { 0x24, 0x2a, 0x7f, 0x2a, 0x12 },  // 24 $
  { 0x23, 0x13, 0x08, 0x64, 0x62 },  // 25 %
  { 0x36, 0x49, 0x55, 0x22, 0x50 },  // 26 &
  { 0x00, 0x05, 0x03, 0x00, 0x00 },  // 27 '
  { 0x00, 0x1c, 0x22, 0x41, 0x00 },  // 28 (
  { 0x00, 0x41, 0x22, 0x1c, 0x00 },  // 29 )
  { 0x14, 0x08, 0x3e, 0x08, 0x14 },  // 2a *
  { 0x08, 0x08, 0x3e, 0x08, 0x08 },  // 2b +
  { 0x00, 0x50, 0x30, 0x00, 0x00 },  // 2c ,
  { 0x08, 0x08, 0x08, 0x08, 0x08 },  // 2d -
  { 0x00, 0x60, 0x60, 0x00, 0x00 },  // 2e .
  { 0x20, 0x10, 0x08, 0x04, 0x02 },  // 2f /
  { 0x3e, 0x51, 0x49, 0x45, 0x3e },  // 30 0
  { 0x00, 0x42, 0x7f, 0x40, 0x00 },  // 31 1
  { 0x42, 0x61, 0x51, 0x49, 0x46 },  // 32 2
  { 0x21, 0x41, 0x45, 0x4b, 0x31 },  // 33 3
  { 0x18, 0x14, 0x12, 0x7f, 0x10 },  // 34 4
  { 0x27, 0x45, 0x45, 0x45, 0x39 },  // 35 5
  { 0x3c, 0x4a, 0x49, 0x49, 0x30 },  // 36 6
  { 0x01, 0x71, 0x09, 0x05, 0x03 },  // 37 7
  { 0x36, 0x49, 0x49, 0x49, 0x36 },  // 38 8
  { 0x06, 0x49, 0x49, 0x29, 0x1e },  // 39 9
  { 0x00, 0x36, 0x36, 0x00, 0x00 },  // 3a :
  { 0x00, 0x56, 0x36, 0x00, 0x00 },  // 3b ;
  { 0x08, 0x14, 0x22, 0x41, 0x00 },  // 3c <
  { 0x14, 0x14, 0x14, 0x14, 0x14 },  // 3d =
  { 0x00, 0x41, 0x22, 0x14, 0x08 },  // 3e >
  { 0x02, 0x01, 0x51, 0x09, 0x06 },  // 3f ?
  { 0x32, 0x49, 0x79, 0x41, 0x3e },  // 40 @
  { 0x7e, 0x11, 0x11, 0x11, 0x7e },  // 41 A
  { 0x7f, 0x49, 0x49, 0x49, 0x36 },  // 42 B
  { 0x3e, 0x41, 0x41, 0x41, 0x22 },  // 43 C
  { 0x7f, 0x41, 0x41, 0x22, 0x1c },  // 44 D
  { 0x7f, 0x49, 0x49, 0x49, 0x41 },  // 45 E
  { 0x7f, 0x09, 0x09, 0x09, 0x01 },  // 46 F
  { 0x3e, 0x41, 0x49, 0x49, 0x7a },  // 47 G
  { 0x7f, 0x08, 0x08, 0x08, 0x7f },  // 48 H
  { 0x00, 0x41, 0x7f, 0x41, 0x00 },  // 49 I
  { 0x20, 0x40, 0x41, 0x3f, 0x01 },  // 4a J
  { 0x7f, 0x08, 0x14, 0x22, 0x41 },  // 4b K
  { 0x7f, 0x40, 0x40, 0x40, 0x40 },  // 4c L
  { 0x7f, 0x02, 0x0c, 0x02, 0x7f },  // 4d M
  { 0x7f, 0x04, 0x08, 0x10, 0x7f },  // 4e N
  { 0x3e, 0x41, 0x41, 0x41, 0x3e },  // 4f O
  { 0x7f, 0x09, 0x09, 0x09, 0x06 },  // 50 P
  { 0x3e, 0x41, 0x51, 0x21, 0x5e },  // 51 Q
  { 0x7f, 0x09, 0x19, 0x29, 0x46 },  // 52 R
  { 0x46, 0x49, 0x49, 0x49, 0x31 },  // 53 S
  { 0x01, 0x01, 0x7f, 0x01, 0x01 },  // 54 T
  { 0x3f, 0x40, 0x40, 0x40, 0x3f },  // 55 U
  { 0x1f, 0x20, 0x40, 0x20, 0x1f },  // 56 V
  { 0x3f, 0x40, 0x38, 0x40, 0x3f },  // 57 W
  { 0x63, 0x14, 0x08, 0x14, 0x63 },  // 58 X
  { 0x07, 0x08, 0x70, 0x08, 0x07 },  // 59 Y
  { 0x61, 0x51, 0x49, 0x45, 0x43 },  // 5a Z
  { 0x00, 0x7f, 0x41, 0x41, 0x00 },  // 5b [
  { 0x02, 0x04, 0x08, 0x10, 0x20 },  // 5c backslash
  { 0x00, 0x41, 0x41, 0x7f, 0x00 },  // 5d ]
  { 0x04, 0x02, 0x01, 0x02, 0x04 },  // 5e ^
  { 0x40, 0x40, 0x40, 0x40, 0x40 },  // 5f _
  { 0x00, 0x01, 0x02, 0x04, 0x00 },  // 60 `
  { 0x20, 0x54, 0x54, 0x54, 0x78 },  // 61 a
  { 0x7f, 0x48, 0x44, 0x44, 0x38 },  // 62 b
  { 0x38, 0x44, 0x44, 0x44, 0x20 },  // 63 c
  { 0x38, 0x44, 0x44, 0x48, 0x7f },  // 64 d
  { 0x38, 0x54, 0x54, 0x54, 0x18 },  // 65 e
  { 0x08, 0x7e, 0x09, 0x01, 0x02 },  // 66 f
  { 0x0c, 0x52, 0x52, 0x52, 0x3e },  // 67 g
  { 0x7f, 0x08, 0x04, 0x04, 0x78 },  // 68 h
  { 0x00, 0x44, 0x7d, 0x40, 0x00 },  // 69 i
  { 0x20, 0x40, 0x44, 0x3d, 0x00 },  // 6a j
  { 0x7f, 0x10, 0x28, 0x44, 0x00 },  // 6b k
  { 0x00, 0x41, 0x7f, 0x40, 0x00 },  // 6c l
  { 0x7c, 0x04, 0x18, 0x04, 0x78 },  // 6d m
  { 0x7c, 0x08, 0x04, 0x04, 0x78 },  // 6e n
  { 0x38, 0x44, 0x44, 0x44, 0x38 },  // 6f o
  { 0x7c, 0x14, 0x14, 0x14, 0x08 },  // 70 p
  { 0x08, 0x14, 0x14, 0x18, 0x7c },  // 71 q
  { 0x7c, 0x08, 0x04, 0x04, 0x08 },  // 72 r
  { 0x48, 0x54, 0x54, 0x54, 0x20 },  // 73 s
  { 0x04, 0x3f, 0x44, 0x40, 0x20 },  // 74 t
  { 0x3c, 0x40, 0x40, 0x20, 0x7c },  // 75 u
  { 0x1c, 0x20, 0x40, 0x20, 0x1c },  // 76 v
  { 0x3c, 0x40, 0x30, 0x40, 0x3c },  // 77 w
  { 0x44, 0x28, 0x10, 0x28, 0x44 },  // 78 x
  { 0x0c, 0x50, 0x50, 0x50, 0x3c },  // 79 y
  { 0x44, 0x64, 0x54, 0x4c, 0x44 },  // 7a z
  { 0x00, 0x08, 0x36, 0x41, 0x00 },  // 7b {
  { 0x00, 0x00, 0x7f, 0x00, 0x00 },  // 7c |
  { 0x00, 0x41, 0x36, 0x08, 0x00 },  // 7d }
  { 0x10, 0x08, 0x08, 0x10, 0x08 },  // 7e ~
  { 0x00, 0x00, 0x00, 0x00, 0x00 }   // 7f
};


#define PCD8544_CMD  0
#define PCD8544_DATA 1


// Chip variant in use...
unsigned char model;

// The size of the Nokia5110_display, in pixels...
static const unsigned char width = 84;
static const unsigned char height = 48;

// Current cursor position...
static unsigned char column = 0;
static unsigned char line = 0;

static const unsigned char *custom[' '];

void Nokia5110_Init()
{
	memset(custom, 0, sizeof(custom));

	Nokia5110p_Init();
    // Set the LCD parameters...
    Nokia5110_send(PCD8544_CMD, 0x21);  // extended instruction set control (H=1)
    Nokia5110_send(PCD8544_CMD, 0x13);  // bias system (1:48)


    Nokia5110_send(PCD8544_CMD, 0xb2);  // default Vop (3.06 + 50 * 0.06 = 6.06V) @3.3V supply


    Nokia5110_send(PCD8544_CMD, 0x20);  // extended instruction set control (H=0)
    Nokia5110_send(PCD8544_CMD, 0x09);  // all Nokia5110_display segments on

    // Clear RAM contents...
    Nokia5110_clear();

    // Activate LCD...
    Nokia5110_send(PCD8544_CMD, 0x08);  // Nokia5110_display blank
    Nokia5110_send(PCD8544_CMD, 0x0c);  // normal mode (0x0d = inverse mode)

	Nokia5110p_Delay(100);


    // Place the cursor at the origin...
    Nokia5110_send(PCD8544_CMD, 0x80);
    Nokia5110_send(PCD8544_CMD, 0x40);

}


void Nokia5110_stop()
{
    Nokia5110_clear();
    Nokia5110_setPower(0);
}


void Nokia5110_clear()
{
    Nokia5110_setCursor(0, 0);

    for (unsigned short i = 0; i < width * (height/8); i++) {
        Nokia5110_send(PCD8544_DATA, 0x00);
    }

    Nokia5110_setCursor(0, 0);
}


void Nokia5110_clearLine()
{
    Nokia5110_setCursor(0, line);

    for (unsigned char i = 0; i < width; i++) {
        Nokia5110_send(PCD8544_DATA, 0x00);
    }

    Nokia5110_setCursor(0, line);
}


void Nokia5110_setPower(unsigned char on)
{
    Nokia5110_send(PCD8544_CMD, on ? 0x20 : 0x24);
}


inline void Nokia5110_display()
{
    Nokia5110_setPower(1);
}


inline void Nokia5110_noDisplay()
{
    Nokia5110_setPower(0);
}


void Nokia5110_setInverse(unsigned char inverse)
{
    Nokia5110_send(PCD8544_CMD, inverse ? 0x0d : 0x0c);
}


void Nokia5110_setContrast(unsigned char level)
{
    // The PCD8544 datasheet specifies a maximum Vop of 8.5V for safe
    // operation in low temperatures, which limits the contrast level.
    if (level > 90) {
        level = 90;  // Vop = 3.06 + 90 * 0.06 = 8.46V
    }

    Nokia5110_send(PCD8544_CMD, 0x21);  // extended instruction set control (H=1)
    Nokia5110_send(PCD8544_CMD, 0x80 | (level & 0x7f));
    Nokia5110_send(PCD8544_CMD, 0x20);  // extended instruction set control (H=0)
}


void Nokia5110_home()
{
    Nokia5110_setCursor(0, line);
}


void Nokia5110_setCursor(unsigned char column, unsigned char line)
{
    column = (column % width);
    line = (line % (height/9 + 1));

    Nokia5110_send(PCD8544_CMD, 0x80 | column);
    Nokia5110_send(PCD8544_CMD, 0x40 | line);
}


void Nokia5110_createChar(unsigned char chr, const unsigned char *glyph)
{
    // ASCII 0-31 only...
    if (chr >= ' ') {
        return;
    }

    custom[chr] = glyph;
}


void Nokia5110_write(unsigned char chr)
{
    // ASCII 7-bit only...
    if (chr >= 0x80) {
        return;
    }

    const unsigned char *glyph;
    unsigned char pgm_buffer[5];

    if (chr >= ' ') {
        // Regular ASCII characters are kept in flash to save RAM...
        memcpy(pgm_buffer, &charset[chr - ' '], sizeof(pgm_buffer));
        glyph = pgm_buffer;
    } else {
        // Custom glyphs, on the other hand, are stored in RAM...
        if (custom[chr]) {
            glyph = custom[chr];
        } else {
            // Default to a space character if unset...
            memcpy(pgm_buffer, &charset[0], sizeof(pgm_buffer));
            glyph = pgm_buffer;
        }
    }

    // Output one column at a time...
    for (unsigned char i = 0; i < 5; i++) {
        Nokia5110_send(PCD8544_DATA, glyph[i]);
    }

    // One column between characters...
    Nokia5110_send(PCD8544_DATA, 0x00);

    // Update the cursor position...
    column = (column + 6) % width;

    if (column == 0) {
        line = (line + 1) % (height/9 + 1);
    }

}

void Nokia5110_Print(const char *str)
{
	int len = strlen(str);
	if(len > 80) len = 80;
	for(int i = 0; i < len; i++)
	{
		Nokia5110_write(*(str+i));
	}
}

void Nokia5110_drawBitmap(const unsigned char *data, unsigned char columns, unsigned char lines)
{
    unsigned char scolumn = column;
    unsigned char sline = line;

    // The bitmap will be clipped at the right/bottom edge of the Nokia5110_display...
    unsigned char mx = (scolumn + columns > width) ? (width - scolumn) : columns;
    unsigned char my = (sline + lines > height/8) ? (height/8 - sline) : lines;

    for (unsigned char y = 0; y < my; y++) {
        Nokia5110_setCursor(scolumn, sline + y);

        for (unsigned char x = 0; x < mx; x++) {
            Nokia5110_send(PCD8544_DATA, data[y * columns + x]);
        }
    }

    // Leave the cursor in a consistent position...
    Nokia5110_setCursor(scolumn + columns, sline);
}


void Nokia5110_drawColumn(unsigned char lines, unsigned char value)
{
    unsigned char scolumn = column;
    unsigned char sline = line;

    // Keep "value" within range...
    if (value > lines*8) {
        value = lines*8;
    }

    // Find the line where "value" resides...
    unsigned char mark = (lines*8 - 1 - value)/8;

    // Clear the lines above the mark...
    for (unsigned char line = 0; line < mark; line++) {
        Nokia5110_setCursor(scolumn, sline + line);
        Nokia5110_send(PCD8544_DATA, 0x00);
    }

    // Compute the byte to draw at the "mark" line...
    unsigned char b = 0xff;
    for (unsigned char i = 0; i < lines*8 - mark*8 - value; i++) {
        b <<= 1;
    }

    Nokia5110_setCursor(scolumn, sline + mark);
    Nokia5110_send(PCD8544_DATA, b);

    // Fill the lines below the mark...
    for (unsigned char line = mark + 1; line < lines; line++) {
        Nokia5110_setCursor(scolumn, sline + line);
        Nokia5110_send(PCD8544_DATA, 0xff);
    }

    // Leave the cursor in a consistent position...
    Nokia5110_setCursor(scolumn + 1, sline);
}


void Nokia5110_send(unsigned char type, unsigned char data)
{
	Nokia5110p_Send(type, data);
}
